        ...
        <div className="flex items-center justify-between h-24">
          ...
          >
            <div className="relative">
              <div className="w-12 h-12 rounded-full bg-gradient-primary flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110">
                <span className="text-white font-bold text-base">JM</span>
              </div>
            ...
            <div className="flex flex-col">
              <span className={`font-display font-bold text-2xl transition-colors duration-300 group-hover:text-primary ${
          ...
          <div className="hidden md:flex items-center space-x-2">
            {navItems.map((item) => (
              <button
                ...
                className={`relative px-5 py-3 rounded-lg transition-all duration-300 font-medium text-base group overflow-hidden ${
                  isScrolled 
          ...
          <button
            ...
            className="md:hidden relative p-4 rounded-xl hover:bg-muted/50 transition-all duration-300 group"
            aria-label="Toggle mobile menu"
          >
            <div className="relative">
              {isOpen ? (
                <X className={`w-7 h-7 transition-colors duration-300 group-hover:text-primary ${
                  isScrolled ? 'text-gray-200' : 'text-foreground'
                }`} />
              ) : (
                <Menu className={`w-7 h-7 transition-colors duration-300 group-hover:text-primary ${
                  isScrolled ? 'text-gray-200' : 'text-foreground'
                }`} />
              )}
          ...
          <div className="py-4 space-y-2 border-t border-border/50">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className={`w-full text-left px-5 py-4 rounded-xl hover:bg-gradient-to-r hover:from-primary/10 hover:to-secondary/10 transition-all duration-300 font-medium text-base group ${
                  isScrolled 
                    ? 'text-gray-200 hover:text-white' 
                    : 'text-foreground hover:text-primary'
                }`}
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className={`block w-full text-left px-5 py-4 rounded-xl hover:bg-gradient-to-r hover:from-primary/10 hover:to-secondary/10 transition-all duration-300 font-medium text-base group ${
                isScrolled 
                  ? 'text-gray-200 hover:text-white' 
                  : 'text-foreground hover:text-primary'
              }`}
